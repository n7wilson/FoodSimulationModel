...
public class AgentFactory {
	...
	private void createRandomAgents(boolean dummy) throws AgentCreationException {
		...
		List<Integer> numAgentTypes = new ArrayList<Integer>();
		if (split.length != 5) {
			throw new AgentCreationException("Problem parsin the definition string '" + this.definition
					+ "': it split into " + split.length + " parts but should split into 5.");
		}
		else{
			// Add an additional number to numAgentTypes to create the 1 work
			numAgentTypes.add(1);
			numAgents++;
		}
		for(String num:split){
		try {
			int agents = Integer.parseInt(num);
			numAgents += agents;
			numAgentTypes.add(agents);
		...
		while (agentsCreated < numAgents) {
			int currentAgentsCreated = 0;
			while (currentAgentsCreated < numAgentTypes.get(agentType)) {
				IAgent a;
				// Create a new agent
				switch(agentType){
					case 1:
				...
				a.setHome(generator.nextDouble()*(GlobalVars.maxPxcor - GlobalVars.minPxcor) + GlobalVars.minPxcor,generator.nextDouble()*(GlobalVars.maxPycor - GlobalVars.minPycor) + GlobalVars.minPycor); // Tell the agent where it lives
				ContextManager.addAgentToContext(a); // Add the agent to the context
				// Finally move the agent to the place where it lives.
				ContextManager.moveAgent(a, geoFac.createPoint(a.getHome()));
				if(a.getClass().equals(Consumer.class)){
					((Consumer)a).setWork();
				}
				agentsCreated++;
				currentAgentsCreated++;
			}
			agentType++;
		}
	}
